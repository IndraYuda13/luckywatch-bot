...
<body>
  ...
  <script>
    ...
    function getPayoutBadge(lastPayout) {
      ...
      const code = String(lastPayout.status_code !== undefined && lastPayout.status_code !== null ? lastPayout.status_code : (lastPayout.status || ''));
        ...
        return `
          <div class="account-card ${isReady ? 'ready-withdraw-border' : ''}">
            ...
            ${(() => {
              if (!acc.last_payout) {
                return `
                  <div class="payout-card-strip" style="background: rgba(255, 255, 255, 0.02); border: 1px dashed var(--border-subtle); border-radius: var(--radius-sm); padding: 7px 10px; display: flex; align-items: center; justify-content: center; font-size: 11px; margin-top: 4px; color: var(--text-tertiary);">
                    <span>No payout history yet</span>
                  </div>
                `;
              }
              const pStyle = getPayoutBadge(acc.last_payout);
              const pId = acc.last_payout.id ? `#${escapeHtml(String(acc.last_payout.id))}` : '';
              const pTime = escapeHtml(String(acc.last_payout.timestamp || ''));
              const pAmount = escapeHtml(String(acc.last_payout.amount || '0.00000'));
              const pNet = acc.last_payout.net_amount ? ` (Net: $${escapeHtml(String(acc.last_payout.net_amount))})` : '';
              const tooltip = escapeHtml(`Payout ${pId} | Status: ${pStyle.label} | Wallet: ${acc.last_payout.wallet || '-'}${pNet}`);
              ...
              return `
                <div class="payout-card-strip" style="background: ${pStyle.bg}; border: 1px solid ${pStyle.border}; border-radius: var(--radius-sm); padding: 7px 10px; display: flex; justify-content: space-between; align-items: center; font-size: 11px; margin-top: 4px;" title="${tooltip}">
                  <div style="display: flex; align-items: center; gap: 6px; min-width: 0;">
                    ${pId ? `<span class="mono" style="font-weight: 700; color: var(--text-secondary); font-size: 11px;">${pId}</span>` : ''}
                    <span class="status-badge-chip" style="background: ${pStyle.bg}; border: 1px solid ${pStyle.border}; color: ${pStyle.color}; padding: 1px 6px; font-size: 9.5px; border-radius: var(--radius-full); text-transform: uppercase; display: inline-flex; align-items: center; gap: 3px;">
                      <span>${pStyle.icon}</span>
                      <span>${pStyle.label}</span>
                    </span>
                  </div>
                  <div style="display: flex; align-items: center; gap: 6px; font-size: 10.5px;" class="mono">
                    <span class="tabular" style="font-weight: 700; color: ${pStyle.color}; font-size: 11.5px;">$${pAmount} USD</span>
                    ${pTime ? `<span style="color: var(--text-tertiary); font-size: 10px;">• ${pTime.split(' ')[1] || pTime}</span>` : ''}
                  </div>
                </div>
              `;
            })()}
        ...
        return `
          <tr style="${isReady ? 'background: rgba(234, 179, 8, 0.04);' : ''}">
            ...
            <td>
              ${(() => {
                if (!acc.last_payout) {
                  return `<span style="color: var(--text-tertiary); font-size: 11px; font-style: italic;">No payout history yet</span>`;
                }
                const pStyle = getPayoutBadge(acc.last_payout);
                const pId = acc.last_payout.id ? `#${escapeHtml(String(acc.last_payout.id))}` : '';
                const pTime = escapeHtml(String(acc.last_payout.timestamp || ''));
                const pAmount = escapeHtml(String(acc.last_payout.amount || '0.00000'));
                const pNet = acc.last_payout.net_amount ? ` (Net: $${escapeHtml(String(acc.last_payout.net_amount))})` : '';
                const tooltip = escapeHtml(`Payout ${pId} | Status: ${pStyle.label} | Wallet: ${acc.last_payout.wallet || '-'}${pNet} | Time: ${pTime}`);

                return `
                  <div style="display: flex; flex-direction: column; gap: 3px; min-width: 130px;" title="${tooltip}">
                    <div style="display: flex; align-items: center; gap: 6px;">
                      <span class="status-badge-chip" style="background: ${pStyle.bg}; border: 1px solid ${pStyle.border}; color: ${pStyle.color}; padding: 1px 6px; font-size: 9.5px; font-weight: 700; border-radius: 4px; text-transform: uppercase; display: inline-flex; align-items: center; gap: 3px;">
                        <span>${pStyle.icon}</span>
                        <span>${pStyle.label}</span>
                      </span>
                      <span class="mono tabular" style="font-weight: 700; color: ${pStyle.color}; font-size: 11.5px;">$${pAmount}</span>
                    </div>
                    <div class="mono" style="font-size: 10px; color: var(--text-tertiary); display: flex; align-items: center; gap: 4px;">
                      ${pId ? `<span style="color: var(--text-secondary); font-weight: 600;">${pId}</span><span>•</span>` : ''}
